...
"""
    ...
    """
        ...
        // 🔹 GESTIÓN DE PENDIENTES (Updated for Order Management)

        @Query("SELECT * FROM orders WHERE mesa = :mesa AND status = 'OPEN' LIMIT 1")
        suspend fun getOpenOrderByTable(mesa: String): OrderEntity?

        @Query("UPDATE orders SET status = 'CLOSED' WHERE orderId = :orderId")
        suspend fun closeOrder(orderId: Long)
