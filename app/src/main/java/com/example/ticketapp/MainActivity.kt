...
class MainActivity : AppCompatActivity() {
    ...
    private var preciosHamburguesas = mutableMapOf(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Aplicar precios editados por el usuario a las hamburguesas
        preciosHamburguesas.keys.toList().forEach { sourceName ->
            val override = catalogConfig.productOverrides[sourceName]
            if (override != null && override.price > 0.0) {
                preciosHamburguesas[sourceName] = override.price
            }
        }
    ...
    private fun ejecutarImpresion(
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                // 2️⃣ Imprimir automáticamente

                val exito =

                        try {

                            viewModel.printTicket(productosSeleccionados, editTextMesa.text.toString().trim())
                ...
                // 3️⃣ Guardar la orden en la base de datos SOLO si se imprimió con éxito
    ...
    private fun setupButtons() {
        ...
        // Restaurar selección de impresora guardada en SharedPreferences
        val prefs = getSharedPreferences("app_settings", MODE_PRIVATE)
        val savedPrinterType = prefs.getString("printer_type", "BT_58") ?: "BT_58"
        when (savedPrinterType) {
            "NET_80" -> {
                radioPrinter80Network.isChecked = true
                printerManager.selectedPrinterType = com.example.ticketapp.PrinterType.NETWORK_80MM
            }
            "NET_58" -> {
                radioPrinter58Network.isChecked = true
                printerManager.selectedPrinterType = com.example.ticketapp.PrinterType.NETWORK_58MM
            }
            else -> {
                radioPrinter58.isChecked = true
                printerManager.selectedPrinterType = com.example.ticketapp.PrinterType.BLUETOOTH_58MM
            }
        }
        ...
        printerTypeGroup.setOnCheckedChangeListener { _, checkedId ->
            printerManager.selectedPrinterType = when (checkedId) {
            ...
            // Guardar selección para que sobreviva recreate() y reinicios
            val prefKey = when (printerManager.selectedPrinterType) {
                com.example.ticketapp.PrinterType.NETWORK_80MM -> "NET_80"
                com.example.ticketapp.PrinterType.NETWORK_58MM -> "NET_58"
                else -> "BT_58"
            }
            getSharedPreferences("app_settings", MODE_PRIVATE).edit().putString("printer_type", prefKey).apply()
                ...
                AlertDialog.Builder(this@MainActivity)
                        ...
                        .setPositiveButton("Imprimir") { _, _ ->
                            ...
                            lifecycleScope.launch(Dispatchers.IO) {
                                ...
                                val printed = viewModel.printTicket(productosSeleccionados, editTextMesa.text.toString().trim())
    ...
    private fun reimprimirTicket(order: OrderEntity) {
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            val productos = items.map {
                Producto(nombre = it.name, precio = it.unitPrice, cantidad = it.quantity, esCombo = it.esCombo)
            }
            ...
            withContext(Dispatchers.Main) {
                ...
                val mesaStr = order.mesa ?: ""
                val isParaLlevar = mesaStr.equals("llevar", ignoreCase = true)

                // ----- Logo y encabezado -----
                val logoView = dialogView.findViewById<android.widget.ImageView>(R.id.ticketLogoImageView)
                if (isParaLlevar) {
                    logoView.visibility = android.view.View.GONE
                }

                // ----- Fecha -----
                val fechaHora = java.text.SimpleDateFormat("yyyy-MM-dd HH:mm", java.util.Locale.getDefault())
                    .format(java.util.Date(order.createdAt))
                dialogView.findViewById<TextView>(R.id.ticketFechaTv).text = "Fecha: $fechaHora"

                // ----- Mesa -----
                val mesaTv = dialogView.findViewById<TextView>(R.id.ticketMesaTv)
                if (mesaStr.isNotEmpty()) {
                    mesaTv.visibility = android.view.View.VISIBLE
                    mesaTv.text = "Mesa/Info: ${mesaStr.uppercase()}"
                }

                // ----- Items dinámicos -----
                val container = dialogView.findViewById<LinearLayout>(R.id.ticketItemsContainer)
                var total = 0.0

                // Separador de platillos
                val normales = productos.filter { !it.esCombo }
                ...
                fun addItemRow(p: Producto) {
                    ...
                    fun tv(txt: String, weight: Float, align: Int): TextView = TextView(this@MainActivity).apply {
                        text = txt
                        typeface = android.graphics.Typeface.MONOSPACE
                        textSize = 12f
                    ...
                    row.addView(tv(p.nombre, 3f, android.view.Gravity.START))
                    row.addView(tv(String.format("$%.2f", p.precio), 1.2f, android.view.Gravity.END))
                    row.addView(tv("x${p.cantidad}", 0.7f, android.view.Gravity.CENTER))
                    row.addView(tv(String.format("$%.2f", subtotal), 1.2f, android.view.Gravity.END))
                    container.addView(row)
                }

                if (normales.isNotEmpty()) {
                    addSectionLabel("— PLATILLOS —")
                    normales.forEach { addItemRow(it) }
                }
                if (combos.isNotEmpty()) {
                    addSectionLabel("— COMBOS —")
                    combos.forEach { addItemRow(it) }
                }

                // ----- Total -----
                dialogView.findViewById<TextView>(R.id.ticketTotalTv).text =
                    String.format("$%.2f", total)

                // ----- QR -----
                val qrImageView = dialogView.findViewById<android.widget.ImageView>(R.id.qrCodeImageView)
                val qrDivider = dialogView.findViewById<android.view.View>(R.id.ticketQrDivider)
                val qrLabel = dialogView.findViewById<TextView>(R.id.ticketQrLabel)
                if (isParaLlevar) {
                    qrImageView.visibility = android.view.View.GONE
                    qrDivider.visibility = android.view.View.GONE
                    qrLabel.visibility = android.view.View.GONE
                } else {
                ...
                btnPrint.setOnClickListener {
                    lifecycleScope.launch(Dispatchers.IO) { viewModel.printTicket(productos, mesaStr) }
                    alertDialog.dismiss()
                }
                val dismissListener = android.view.View.OnClickListener { alertDialog.dismiss() }
